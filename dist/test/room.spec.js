...
// })
ava_1.test('Room smoking test', (t) => __awaiter(this, void 0, void 0, function* () {
    ...
    const mockContactGetter = function (id) {
        return new Promise((resolve, reject) => {
            if (id !== EXPECTED.id && !(id in CONTACT_LIST))
                return resolve({});
            if (id === EXPECTED.id) {
                setTimeout(() => {
    ...
    const r = new room_1.default(EXPECTED.id);
    t.is(r.id, EXPECTED.id, 'should set id/UserName right');
    let puppet;
    try {
        puppet = config_1.default.puppetInstance();
        puppet.getContact = mockContactGetter;
    }
    catch (err) {
        puppet = { getContact: mockContactGetter };
        config_1.default.puppetInstance(puppet);
    }
    yield r.ready();
    ...
    const contact1 = new contact_1.default(EXPECTED.memberId1);
    ...
    const contact2 = new contact_1.default(EXPECTED.memberId2);
    ...
    const noSuchContact = new contact_1.default('not exist id');
    t.false(r.has(noSuchContact), 'should has no this member');
    const owner = r.owner();
    t.true(owner === null || owner instanceof contact_1.default, 'should get Contact instance for owner, or null');
...
ava_1.test('Room static method', (t) => __awaiter(this, void 0, void 0, function* () {
    try {
        const result = yield room_1.default.find({ topic: 'xxx' });
        t.is(result, null, `should return null if cannot find the room`);
    }
    catch (e) {
        t.pass('should throw before login or not found');
    }
    const roomList = yield room_1.default.findAll({
        topic: 'yyy',
    });
